...
describe('BackendSync', () => {
  ...
  it('flushes a sealed session via submit_session_auth and marks it synced', async () => {
    await putSession(sealedSession('a'))
    const { client, rpc } = mockClient()
    const sync = new BackendSync(client)

    await sync.flush()

    expect(rpc).toHaveBeenCalledTimes(1)
    const [fn, args] = rpc.mock.calls[0]
    expect(fn).toBe('submit_session_auth')
    expect(args.p_session.id).toBe('a')
    expect(args.p_session.event_count).toBe(2)
    // No class code / pseudonym are sent — the RPC resolves the student by auth.uid().
    expect(args.p_class_code).toBeUndefined()
    expect(args.p_student_pseudonym).toBeUndefined()
  ...
  it('keeps a session queued when the RPC errors (offline)', async () => {
    ...
    const sync = new BackendSync({ rpc } as never)
    await sync.flush()
    expect(await getUnsynced()).toHaveLength(1)
  })

  it('warns (not silently) when the student is not enrolled', async () => {
    await putSession(sealedSession('a'))
    const rpc = vi.fn().mockResolvedValue({ data: null, error: new Error('not enrolled') })
    const sync = new BackendSync({ rpc } as never)
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})
    await sync.flush()
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('not enrolled'))
    expect(await getUnsynced()).toHaveLength(1)
    warn.mockRestore()
  })
